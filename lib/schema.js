...
class Schema {
	...
	_traverseSubschema(subschema, path, handlers, options) {
		...
		if (handlers.onSubschema) {
			handlers.onSubschema(subschema, path, subschemaType);
		}
		subschemaType.traverseSchema(subschema, path, handlers, options);
	}
	...
	_traverseSubschemaValue(value, subschema, field, handlers) {
		if (subschema) {
			...
			if (handlerResult !== false) {
				subschemaType.traverseObject(value, subschema, field, handlers, this);
			}
		} else {
	...
	_transformSubschemaValue(value, subschema, field, handlers) {
		...
		if (subschema) {
			let subschemaType = this._getType(subschema.type);
			if (handlers.onField) {
				newValue = handlers.onField(field, value, subschema, subschemaType);
			}
			subschemaType.transform(newValue, subschema, field, handlers, this);
			if (handlers.postField) {
				newValue = handlers.postField(field, value, subschema, subschemaType);
			}
		} else {
			if (handlers.onUnknownField) {
	...
	_transformSubschemaValueAsync(value, subschema, field, handlers) {
		if (subschema) {
			...
			promise = promise.then( newValue => {
				return subschema.transformAsync(newValue, subschema, field, handlers, this).then( () => newValue );
			} );
			if (handlers.postField) {
		...
		this.traverse(value, {
			onField(field, value, subschema, subschemaType) {
				if (value === undefined || value === null) {
					if (subschema.required && !options.allowMissingFields) {
						fieldErrors.push(new FieldError('required', 'Field is required', field));
					}
				} else {
					if (Array.isArray(subschema.enum) && !subschemaType.checkEnum(value, subschema.enum)) {
						...
						try {
							subschemaType.validate(value, subschema, field, options, self);
							if (_.isFunction(subschema.validate)) {
								subschema.validate(value, subschema, field, options, self);
							}
						} catch (ex) {
			...
			onUnknownField(field, value) {
				if (!options.allowUnknownFields) {
					fieldErrors.push(new FieldError('unknown_field', 'Unknown field', field));
				}
			}
		});
	...
	normalize(value, options = {}) {
		let self = this;
		let fieldErrors = [];
		let normalizedValue = this.transformObject(value, {
			onField(field, value, subschema, subschemaType) {
				if (value === undefined || value === null) {
					if (subschema.default !== undefined && subschema.default !== null) {
						value = subschema.default;
					} else if (subschema.required && !options.allowMissingFields) {
						fieldErrors.push(new FieldError('required', 'Field is required', field));
					}
				}
				if (value !== undefined && value !== null) {
					try {
						value = subschemaType.normalize(value, subschema, field, options, self);
						if (_.isFunction(subschema.normalize)) {
							value = subschema.validate(value, subschema, field, options, self);
						}
					} catch (ex) {
						if (FieldError.isFieldError(ex)) {
							ex.field = field;
