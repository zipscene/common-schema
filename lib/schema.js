...
class Schema {
	 ...
	 * @param {Object} options
	 */
	_traverseSubschema(subschema, path, handlers, options) {
		let result;
		let subschemaType = this._getType(subschema.type);
		if (handlers.onSubschema) {
			result = handlers.onSubschema(subschema, path, subschemaType);
		}
		if (result !== false) {
			subschemaType.traverseSchema(subschema, path, handlers, this, options);
		}
	}
	...
	_normalizeSubschema(subschema) {
		if (_.isPlainObject(subschema) && _.isString(subschema.type)) {
			...
			schemaType.normalizeSchema(subschema, this);
			return subschema;
		} else if (_.isPlainObject(subschema) && subschema.type) {
	 ...
	 *
	 * @method filterSchema
	 * @param {Function} callback - Called for each subschema. Return true to include the subschema
	 *   and all of its descendants in the result, false to determine inclusion of children
	 *   individually. If a subschema has no included children and false is returned, it will not
	 *   be included.
	 *   @param {Object} callback.subschema - Subschema object
	 *
	 * @return {Schema} - A copy of the schema, without subschemas that fail the test.
	 */
	filterSchema(callback) {
		let filteredObj = {};
		this.traverseSchema({
			onSubschema(subschema, path) {
				if (callback(subschema)) {
					// Handle arrays using shorthand
					path = path.replace('.$', '.0');

					// Ensure arrays in path exist
					let arrayPathComponents = _(path.split('.0')).initial();
					arrayPathComponents.forEach((component, index) => {
						let arrayPath = arrayPathComponents.take(index + 1).join('.0');
						if (!objtools.getPath(filteredObj, arrayPath)) {
							objtools.setPath(filteredObj, arrayPath, []);
						}
					}).value();

					// Include entire subschema
					objtools.setPath(filteredObj, path, subschema);
		...
		return new Schema(filteredObj, this._schemaFactory);
	}
	...
	/**
	 * A duck-type tests for whether a value is a Schema instance
	 *
	 * @method isSchema
	 * @static
	...
	static isSchema(value) {
		return (
			_.isObject(value) &&
			_.isFunction(value.traverseSchema) &&
			_.isFunction(value.normalizeSchema) &&
			_.isFunction(value.getData) &&
			_.isFunction(value.traverse) &&
			_.isFunction(value.transform) &&
			_.isFunction(value.transformAsync) &&
			_.isFunction(value.validate) &&
			_.isFunction(value.normalize) &&
			_.isFunction(value.serialize) &&
			_.isFunction(value.createValidateFn) &&
			_.isFunction(value.createNormalizeFn) &&
			_.isFunction(value.getSchemaType) &&
			_.isFunction(value.getSubschemaData)
		);
	}
