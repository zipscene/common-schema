...
Schema.prototype.traverseDoc = function(doc, callback) {
	function traverse(subdoc, subschema, path) {
		...
		if(typeof subschema.schemaTraverse == 'function') {
			subschema.schemaTraverse(subdoc, subschema, path, traverse);
		} else if(documentTraversers[subschema.type]) {
			documentTraversers[subschema.type](subdoc, subschema, path, traverse);
		}
	}
...
Schema.prototype.transformDoc = function(doc, callback) {
	function transform(subdoc, subschema, path) {
		...
		if(typeof subschema.schemaTransform == 'function') {
			newValue = subschema.schemaTransform(subdoc, subschema, path, transform);
		} else if(documentTransformers[subschema.type]) {
			newValue = documentTransformers[subschema.type](subdoc, subschema, path, transform);
		}
		return newValue;
	}
	...
	function traverse(subdoc, subschema, path, done) {
		...
		callback(subdoc, subschema, path, function(error) {
			...
			if(typeof subschema.schemaTraverseAsync == 'function') {
				subschema.schemaTraverseAsync(subdoc, subschema, path, traverse, done);
			} else if(asyncDocumentTraversers[subschema.type]) {
				asyncDocumentTraversers[subschema.type](subdoc, subschema, path, traverse, done);
			} else {
				done();
			}
		});
	...
	function transform(subdoc, subschema, path, done) {
		...
		callback(subdoc, subschema, path, function(error, newValue) {
			...
			if(typeof subschema.schemaTransformAsync == 'function') {
				subschema.schemaTransformAsync(newValue, subschema, path, transform, done);
			} else if(asyncDocumentTransformers[subschema.type]) {
				asyncDocumentTransformers[subschema.type](newValue, subschema, path, transform, done);
			} else {
				done(null, newValue);
			}
		});
...
Schema.prototype.normalizeDoc = function(doc) {
	...
	return this.transformDoc(doc, function(value, subschema, path) {
		...
		if(typeof subschema.prevalidate == 'function' && value !== null && value !== undefined) {
			subschema.prevalidate(value, subschema, path);
		}
		if(defaultNormalizers[type] && value !== null && value !== undefined) {
			value = defaultNormalizers[type](value, subschema, path);
		}
		if(typeof subschema.schemaNormalizeValue == 'function' && value !== null && value !== undefined) {
			value = subschema.schemaNormalizeValue(value, subschema, path);
		}
		if(extraNormalizers[type] && value !== null && value !== undefined) {
			value = extraNormalizers[type](value, subschema, path);
		}
		if(typeof subschema.validate == 'function' && value !== null && value !== undefined) {
			subschema.validate(value, subschema, path);
		}
		if(typeof subschema.normalize == 'function' && value !== null && value !== undefined) {
			value = subschema.normalize(value, subschema, path);
		}
		commonValidate(value, subschema, path);
		return value;
	});
};
