...
var defaultTypeSchemaNormalizers = Schema.defaultTypeSchemaNormalizers = {
...
var documentTraversers = Schema.documentTraversers = {
...
var documentTransformers = Schema.documentTransformers = {
...
var schemaTraversers = Schema.schemaTraversers = {
...
var asyncDocumentTraversers = Schema.asyncDocumentTraversers = {
...
var asyncDocumentTransformers = Schema.asyncDocumentTransformers = {
...
var subschemaGetters = Schema.subschemaGetters = {
...
Schema.prototype._normalizeAndValidateSchemaData = function() {
	...
	function normalizeSubschema(subschema, path) {
		...
		if(Array.isArray(subschema.type)) {
			...
			for(var key in subschema.type) {
				if(subschema.type.hasOwnProperty(key)) {
					subschema.properties[key] = subschema.type[key];
				}
			}
		...
		if(typeof subschema.type != 'string') throw new ZSError(ZSError.INVALID_OBJECT, 'Invalid schema.  Type must either be a string or one of the recognized shorthand types.', { givenType: subschema.type, path: path });
		if(!newTypeFromTypeMapper && options.allowedTypes.indexOf(subschema.type) == -1) throw new ZSError(ZSError.INVALID_OBJECT, 'Invalid schema.  Given type is not recognized or disallowed.', { givenType: subschema.type, path: path });
...
Schema.prototype.transformDoc = function(doc, callback) {
	...
	function transform(subdoc, subschema, path) {
		...
		if(typeof subschema.schemaTransform == 'function') {
			newValue = subschema.schemaTransform.call(schema, newValue, subschema, path, transform);
		} else if(documentTransformers[subschema.type]) {
			newValue = documentTransformers[subschema.type].call(schema, newValue, subschema, path, transform);
		}
		return newValue;
	}
...
require('./map').registerType(Schema);
